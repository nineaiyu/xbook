...
class LocalCreate(BaseAligo):
    """创建文件: 1.创建文件 2.上传文件 3.下载文件"""
    # 控制内存消耗 100M 左右，但默认单文件上传只支持 1T，如需更大，手动 调节 LocalCreate._UPLOAD_CHUNK_SIZE 的值
    ...
    def _get_part_info_list(file_size: int) -> List[UploadPartInfo]:
        ...
        # 以10MB为一块: 10485760
        return [UploadPartInfo(part_number=i) for i in range(1, math.ceil(file_size / LocalCreate.__UPLOAD_CHUNK_SIZE) + 1)]

    def _local_pre_hash(self, file_path: str, file_size: int, name: str, parent_file_id='root', drive_id=None,
    ...
    def _local_get_proof_code(self, file_path: str, file_size: int) -> str:
        """计算proof_code"""
        md5_int = int(hashlib.md5(self._auth.drive_obj.access_token.encode()).hexdigest()[:16], 16)
    ...
    def _local_content_hash(self, file_path: str, file_size: int, name: str, parent_file_id='root', drive_id=None,
                      check_name_mode: CheckNameMode = 'auto_rename') -> CreateFileResponse:
        ...
        proof_code = self._local_get_proof_code(file_path, file_size)
        ...
        if response.status_code == 400:
            body.proof_code = self._local_get_proof_code(file_path, file_size)
            response = self._post(ADRIVE_V2_FILE_CREATEWITHFOLDERS, body=body)
    ...
    def _local_put_data(self, file_path: str, part_info: CreateFileResponse, file_size: int) -> Union[BaseFile, Null]:
        """上传数据"""
        with open(file_path, 'rb') as f:
            ...
            for i in range(len(part_info.part_info_list)):
                ...
                data = f.read(LocalCreate.__UPLOAD_CHUNK_SIZE)
                try:
    ...
    def local_upload_file(
        ...
        >>> from common.libs.alidrive import Aligo
        >>> ali = Aligo()
        >>> up_file = ali.local_upload_file('/Users/aligo/Desktop/test.txt')
        >>> print(up_file)
        """
        ...
        if LocalCreate._UPLOAD_CHUNK_SIZE is None:
            if file_size < 104857600000:  # (1024 * 1024 * 10) * 10000
                LocalCreate.__UPLOAD_CHUNK_SIZE = 10485760  # (1024 * 1024 * 10) => 10 MB
            else:
                LocalCreate.__UPLOAD_CHUNK_SIZE = 268435456  # 256 MB
        else:
            if file_size < LocalCreate._UPLOAD_CHUNK_SIZE * 10000:
                LocalCreate.__UPLOAD_CHUNK_SIZE = LocalCreate._UPLOAD_CHUNK_SIZE
            else:
                LocalCreate.__UPLOAD_CHUNK_SIZE = 268435456  # 256 MB

        if file_size > 1024:  # 1kB
            # 1. pre_hash
            part_info = self._local_pre_hash(file_path=file_path, file_size=file_size, name=name,
                                       parent_file_id=parent_file_id, drive_id=drive_id,
                                       check_name_mode=check_name_mode)

            if part_info.code == 'PreHashMatched':
                # 2. content_hash
                part_info = self._local_content_hash(file_path=file_path, file_size=file_size, name=name,
                                               parent_file_id=parent_file_id, drive_id=drive_id,
                                               check_name_mode=check_name_mode)
                if part_info.rapid_upload:
                    ...
                    return part_info
            # 开始上传
            # return self._local_put_data(file_path, part_info, file_size)
        else:
            # 2. content_hash
            part_info = self._local_content_hash(file_path=file_path, file_size=file_size, name=name,
                                           parent_file_id=parent_file_id, drive_id=drive_id,
    ...
    def create_by_hash(
        ...
        proof_code = self._local_get_proof_code(url, size)
        body = CreateFileRequest(
            name=name,
            content_hash=content_hash,
            size=size,
