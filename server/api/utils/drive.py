...
from api.models import AliyunDrive, AliyunFileInfo
...
def check_file_punish(file_obj: AliyunFileInfo):
...
def batch_get_download_url(file_obj_list: [AliyunFileInfo]) -> list:
    cache_result = []
    drive_obj_dict = {}
...
def batch_delete_file(file_obj_list: [AliyunFileInfo]) -> bool:
    drive_obj_dict = {}
    for file_obj in file_obj_list:
        drive_user_id = file_obj.aliyun_drive_id.user_id
        drive_obj = drive_obj_dict.get(drive_user_id)
