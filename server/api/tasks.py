...
def clean_visitor_user():
    ...
    user_queryset = UserInfo.objects.filter(is_active=True, is_superuser=False, is_staff=False, last_name='0',
                                            last_login__lte=value)
    file_user_queryset = user_queryset.filter(fileinfo__isnull=False)

    items = AliyunFileInfo.objects.filter(owner_id__in=file_user_queryset).values('owner_id__username',
